...
export default function Login() {
  ...
  const { setIsAdmin } = useGlobalApi()

  
  const handleSubmit = async ev => {
    ev.preventDefault()
    setLoading(true)
    const fields = { email:emailRef.current.value, password:passwordRef.current.value}
    try {
      const result = await axiosInstance.post('/auth/login', fields).then(res => res)
    ...
    <div className="login-container h-screen fixed inset-0 bg-cl3 flex items-center justify-center z-50">
      <div className="bg-cl5 md:w-[30%] w-[90%] p-8 rounded-md">
        <div className="text-center text-white mb-4">
          ...
          {message && <span className="block text-center bg-red-500 py-1 mt-3">{message}</span>}
          {isLoading && 'Loading...'}
        </div>
